...
# Hello World`);
  ...
  it('リポジトリに記事が存在しない場合、NOT_FOUNDエラーを投げること', async () => {
    ...
    // Act & Assert
    await expect(getArticle(mockCategory, mockSlug))
      .rejects
      .toThrow(new ArticleServiceError('NOT_FOUND', 'Article not found'));
  });
  ...
  it('ファイルの読み込みに失敗した場合、INTERNAL_ERRORを投げること', async () => {
    ...
    await expect(getArticle(mockCategory, mockSlug))
      .rejects
      .toThrow(ArticleServiceError);

    await expect(getArticle(mockCategory, mockSlug))
      .rejects
      .toThrow('Failed to load article content');
  });
});
